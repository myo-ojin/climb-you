...
class AIInitializationService {
  ...
  async initializeAllServices(): Promise<AIInitializationResult> {
    ...
    // API Key診断
    console.log('🔍 Starting API key diagnosis...');
    const diagnosis = apiKeyManager.diagnoseConfiguration();
    console.log('🔑 API Key Diagnosis Result:', {
      openaiKeyConfigured: diagnosis.openaiKeyConfigured,
      openaiKeyValid: diagnosis.openaiKeyValid,
      aiFeatureEnabled: diagnosis.aiFeatureEnabled,
      debugMode: diagnosis.debugMode,
      mode: diagnosis.aiFeatureEnabled ? 'PRODUCTION' : 'DEMO'
    });
    ...
    if (!result.apiKeyStatus.valid) {
      ...
      console.warn('⚠️  OpenAI API key not configured or invalid - Running in DEMO mode');
      console.log('📝 Setup Instructions:', result.apiKeyStatus.setupInstructions);
    } else {
      console.log('✅ OpenAI API key is valid - Production mode enabled');
    }
