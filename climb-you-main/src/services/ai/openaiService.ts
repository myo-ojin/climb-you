...
`;
  ...
  async generateSkillMap(goalText: string, currentLevelTags?: string[], priorityAreas?: string[]): Promise<any[]> {
    const prompt = `
あなたは専門的な学習デザイナーです。以下の学習目標に基づいて、段階的なスキル習得マップを生成してください。

学習目標: ${goalText}
${currentLevelTags ? `現在のレベル: ${currentLevelTags.join(', ')}` : ''}
${priorityAreas ? `優先分野: ${priorityAreas.join(', ')}` : ''}

以下の形式でスキルアトムを8-12個生成してください：
- id: ユニークな識別子
- label: スキル名（日本語）
- type: "concept"（概念）, "procedure"（手順）, "habit"（習慣）のいずれか
- level: "intro"（導入）, "basic"（基礎）, "intermediate"（中級）のいずれか
- bloom: "understand"（理解）, "apply"（適用）, "analyze"（分析）, "create"（創造）のいずれか
- prereq: 前提となるスキルのIDリスト
- representative_tasks: このスキルを習得するための代表的なタスク
- suggested_patterns: 学習パターン（"read_note_q", "build_micro", "flashcards", "past_paper"から選択）
...
[
  {
    ...
    "bloom": "understand",
    "prereq": [],
    "representative_tasks": ["基礎資料を読み、要点をまとめる"],
    "suggested_patterns": ["read_note_q"]
  }
]
`;
    ...
    const response = await this.makeRequest('/chat/completions', {
      model: API_CONFIG.OPENAI_MODEL,
      messages: [
        { role: 'system', content: 'あなたは専門的な学習デザイナーです。学習目標に基づいて体系的なスキルマップを作成してください。' },
        { role: 'user', content: prompt }
      ],
      temperature: 0.7,
      max_tokens: 3000,
    ...
    try {
      ...
      console.error('Failed to parse skill map:', error);
